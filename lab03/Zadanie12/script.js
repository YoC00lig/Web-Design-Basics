...
class Zombie {
    ...
    constructor() {
        ...
        this.y = Math.random() * (canvas.height - canvas.height/2 - 2*this.height) + canvas.height/2 + this.height;  // to avoid having zombie parially hidden (generated out of screen height) - halfway down the screen
        // to change locaton at time unit - delta x and delta y
        this.dx = Math.random() * (10-3) + 3; // random number between 3 and 10
        this.delete = false;
...
async function addScore() {
    ...
    const rawResponse = await fetch("http://localhost:3000/scores?", {
          ...
          body: JSON.stringify({id: response.length + 1, name: nick, score: score})
        });
}

async function buildHighScores() {
    const response = await fetch("http://localhost:3000/scores?");
    const data = await response.json();
    data.sort((a,b) => b.score - a.score);
    firstPlace.innerHTML = "1. " + data[0].name + " " + data[0].score;
    secondPlace.innerHTML = "2. " + data[1].name + " " + data[1].score;
    thirdPlace.innerHTML = "3. " + data[2].name + " " + data[2].score;
}
